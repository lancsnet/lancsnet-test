...
## Fix: Impossible Travel Login Issue

**Symptoms:** Browser bị logout sau 60s, gateway log có `anomaly_revoke: impossible_travel`

**Root cause:** Các background scripts (`scan_watcher.sh`, `auto_remediate.sh`) login với `test@lancs.local` từ `127.0.0.1`, trong khi browser dùng IP public → gateway detect impossible_travel → revoke all tokens

**Fix applied:**
1. Tạo user `automation@lancs.local` cho background scripts
2. Update tất cả scripts trong `/tmp` và `bin/` dùng user mới
3. Xóa audit_log IP history cũ của `test@lancs.local`

**Prevention:**
- Background scripts KHÔNG được dùng cùng user với browser login
- Admin user cho browser: `test@lancs.local`  
- Automation user cho scripts: `automation@lancs.local`
