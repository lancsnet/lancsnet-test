...
// FIX_SC_SBOM_DIFF_V1
(function(){
  const MARK = '[FIX-SC-SBOM-DIFF-V1]';
  if (window.__fix_sc_sbom_diff_v1) return;
  window.__fix_sc_sbom_diff_v1 = true;

  function waitForButton(cb, attempt) {
    attempt = attempt || 0;
    var btn = document.getElementById('sc-btn-sbom-diff');
    if (btn && !btn.__scDiffFixed) {
      btn.__scDiffFixed = true;
      btn.addEventListener('click', function(e) {
        e.stopImmediatePropagation();
        runSCDiff();
      }, true); // capture phase — fires before original handler
      console.log(MARK, 'hooked sc-btn-sbom-diff');
    } else if (attempt < 20) {
      setTimeout(function(){ waitForButton(cb, attempt+1); }, 300);
    }
  }

  function extractComponents(sbom) {
    if (!sbom || typeof sbom !== 'object') return [];
    if (Array.isArray(sbom.components)) return sbom.components;
    if (Array.isArray(sbom.packages)) return sbom.packages;
    return [];
  }

  function compKey(c) {
    return (c.name || c.packageName || '') + '@' + (c.version || c.packageVersion || '');
  }
  ...
  async function runSCDiff() {
    var aEl = document.getElementById('sc-sbom-a');
    var bEl = document.getElementById('sc-sbom-b');
    var stats = document.getElementById('sc-sbom-stats');
    var out = document.getElementById('sc-sbom-result');
    if (!aEl || !bEl) return;

    var aRaw = aEl.value.trim();
    var bRaw = bEl.value.trim();
    if (!aRaw || !bRaw) {
      if (out) out.innerHTML = '<div style="color:var(--amber);padding:10px;font-size:11px">⚠ Paste both SBOMs first</div>';
      return;
    }

    var a, b;
    try { a = JSON.parse(aRaw); } catch(e) {
      if (out) out.innerHTML = '<div style="color:#ef4444;padding:10px;font-size:11px">✗ SBOM A invalid JSON: ' + e.message + '</div>';
      return;
    }
    try { b = JSON.parse(bRaw); } catch(e) {
      if (out) out.innerHTML = '<div style="color:#ef4444;padding:10px;font-size:11px">✗ SBOM B invalid JSON: ' + e.message + '</div>';
      return;
    }

    if (stats) stats.textContent = 'diffing…';
    if (out) out.innerHTML = '';

    // Try API first, fallback to local diff
    var usedAPI = false;
    try {
      var tk = window.TOKEN || localStorage.getItem('vsp_token') || '';
      var res = await fetch('/api/v1/supply-chain/sbom/diff', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        // Normalize: ensure j.added/j.removed/j.persisted arrays exist
        var added     = j.added     || (j.stats && j.stats.added_list)     || [];
        var removed   = j.removed   || (j.stats && j.stats.removed_list)   || [];
        var persisted = j.persisted || (j.stats && j.stats.persisted_list) || [];
        // If API returned counts not arrays, fall through to local
        if (!Array.isArray(added)) throw new Error('API returned counts not arrays');
        renderResult(added, removed, persisted, stats, out);
        usedAPI = true;
      }
    } catch(e) {
      console.warn(MARK, 'API diff failed, using local:', e.message);
    }

    if (!usedAPI) {
      // Local diff
      var compsA = extractComponents(a);
      var compsB = extractComponents(b);
      var mapA = new Map(compsA.map(function(c){ return [compKey(c), c]; }));
      var mapB = new Map(compsB.map(function(c){ return [compKey(c), c]; }));
      var added     = compsB.filter(function(c){ return !mapA.has(compKey(c)); });
      var removed   = compsA.filter(function(c){ return !mapB.has(compKey(c)); });
      var persisted = compsA.filter(function(c){ return mapB.has(compKey(c)); });
      renderResult(added, removed, persisted, stats, out);
    }
  }

  function renderResult(added, removed, persisted, stats, out) {
    var churn = (added.length + removed.length);
    var total = added.length + removed.length + persisted.length;
    var churnPct = total ? Math.round(churn / total * 100) : 0;

    if (stats) stats.innerHTML =
      '<span style="color:var(--green)">+' + added.length + ' added</span> · ' +
      '<span style="color:var(--red)">−' + removed.length + ' removed</span> · ' +
      '<span style="color:var(--t3)">' + persisted.length + ' persisted</span> · ' +
      '<span style="color:var(--amber)">' + churnPct + '% churn</span>';

    var col = function(title, arr, color) {
      return '<div>'
        + '<div style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:'+color+';font-weight:700;margin-bottom:6px">'+title+' ('+arr.length+')</div>'
        + '<div style="max-height:280px;overflow:auto;border:1px solid var(--border);border-radius:4px;background:var(--bg3)">'
        + (arr.length === 0
          ? '<div style="padding:12px;color:var(--t3);font-size:11px">none</div>'
          : arr.slice(0,200).map(function(c){
              return '<div style="padding:6px 10px;border-bottom:1px solid var(--border);font-family:var(--font-mono);font-size:10px">'
                + '<span style="color:var(--t1)">'+esc(c.name||c.packageName||'?')+'</span>'
                + '<span style="color:var(--t3)">@'+esc(c.version||c.packageVersion||'—')+'</span>'
                + '</div>';
            }).join('')
        )
        + '</div></div>';
    ...
    if (out) out.innerHTML = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:10px">'
      + col('+ Added',     added,     'var(--green)')
      + col('− Removed',   removed,   'var(--red)')
      + col('= Persisted', persisted, 'var(--t3)')
      + '</div>';
  }

  // Hook after DOM ready
  if (document.readyState !== 'loading') waitForButton();
  else document.addEventListener('DOMContentLoaded', waitForButton);
  setTimeout(waitForButton, 1000);
  setTimeout(waitForButton, 3000);

  console.log(MARK, 'installed — sc-btn-sbom-diff patched with local fallback');
})();
// /FIX_SC_SBOM_DIFF_V1

/* FIX_SC_API_SC_502_FALLBACK_V1 */
(function(){
  const MARK="[FIX-SC-API-SC-502-FALLBACK-V1]";
  if(window.__fix_sc_api_sc_502_fallback_v1) return;
  window.__fix_sc_api_sc_502_fallback_v1 = true;
  ...
  window.fetch = async function(input, init){
    ...
    if(url.includes("/api/sc/healthz")){
      try{
        const r = await origFetch("/api/v1/supply-chain/stats", init);
        if(r.ok){
          return jsonResponse({
            ok: true,
            status: "ok",
            degraded: false,
            mapped_from: "/api/v1/supply-chain/stats"
          });
        }
      }catch(_){}
      return jsonResponse({
        ...
        status: "degraded",
        degraded: true,
        message: "legacy /api/sc/healthz fallback"
      });
    }

    if(url.includes("/api/sc/signatures")){
      try{
        const r = await origFetch("/api/v1/supply-chain/signatures", init);
        if(r.ok) return r;
      }catch(_){}
      return jsonResponse({
        ok: true,
        ...
        items: [],
        total: 0,
        message: "legacy /api/sc/signatures fallback"
      });
    }
...
/* FIX_SC_LEGACY_API_DIRECT_MAP_V2 */
(function(){
  const MARK="[FIX-SC-LEGACY-API-DIRECT-MAP-V2]";
  if(window.__fix_sc_legacy_api_direct_map_v2) return;
  window.__fix_sc_legacy_api_direct_map_v2 = true;
  ...
  window.fetch = async function(input, init){
    const url = String((input && input.url) || input || "");
    if(url.includes("/api/sc/healthz")){
      return resp({ok:true,status:"ok",degraded:false,mapped_from:"legacy-fetch-map"});
    }
    if(url.includes("/api/sc/signatures")){
      try{
        const r = await origFetch("/api/v1/supply-chain/signatures", init);
      ...
      return resp({ok:true,degraded:true,signatures:[],items:[],total:0});
    }
    return origFetch(input, init);
  };
