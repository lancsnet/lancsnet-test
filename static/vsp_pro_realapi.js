...
function renderTenantsReal(root, listResp, quota){
  ...
  var isAdmin = (role === 'superadmin' || role === 'super_admin' || role === 'platform_admin' || role === 'owner');
  ...
  if (isAdmin) {
    root.querySelectorAll('.pro-plan-select').forEach(function(sel){
      sel.addEventListener('change', function(){
        ...
        var next = sel.value;
        if (next === prev) return;
        // RBAC_PLAN_UI_V1: only superadmin/platform admin may change tenant plans.
        if (!canEditTenantPlanV1()) {
          if (typeof PRO.toast === 'function') PRO.toast('Bạn không có quyền thay đổi plan tenant', 'warn');
          else console.warn('[VSP] plan change blocked by UI RBAC: insufficient role');
          return;
        }
...
// RBAC_PLAN_UI_V1: controlled handling for tenant plan 403.
window.addEventListener('unhandledrejection', function(ev) {
  try {
    var msg = String((ev.reason && (ev.reason.message || ev.reason.error)) || ev.reason || '');
    if (msg.indexOf('/api/v1/admin/tenants/') >= 0 && msg.indexOf('/plan') >= 0 && msg.indexOf('403') >= 0) {
      console.warn('[VSP] tenant plan change forbidden handled');
      ev.preventDefault();
    }
  } catch (_) {}
});
