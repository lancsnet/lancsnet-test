...
(function () {
  ...
  if (window.VSP_DEBUG) console.log('[H3.Upgrade] booting — auth:', getToken() ? 'ok' : 'missing');
  ...
  async function injectPRWidget() {
    ...
    // FIX 2026-05-07: skip injection when the anchor is in a non-Autofix
    // tab (the AI Pre-compute card lives in the Autofix sub-tab; rendering
    // the H3.S sibling there would float over Gate/Runs cards on those
    // other tabs because the panel-switcher only toggles tab-panel visibility,
    // not absolutely-positioned siblings inside scrollable parents).
    const tabPanel = anchorCard.closest('.tab-panel, [class*="tab-panel"], [data-tab]');
    if (tabPanel && (tabPanel.id !== 'tab-autofix') && !tabPanel.classList.contains('active')) {
      // The widget is allowed only when sitting inside the Autofix sub-tab.
      // We could insert it later when user activates that tab, but the
      // information is already surfaced inline (gate policy config row +
      // run history table), so dropping the duplicate keeps the layout clean.
      return;
    }
    ...
    widget.style.cssText =
      'margin-top:14px;padding:12px 14px;background:rgba(15,23,42,.6);' +
      'border:1px solid rgba(148,163,184,.15);border-radius:8px;' +
      // Force the widget into normal block flow so it can't float over
      // sibling cards on tab change.
