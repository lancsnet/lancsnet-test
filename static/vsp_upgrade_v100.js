...
// FIX: window.api polyfill
if (typeof window.api !== 'function') {
  window.api = async function(method, url) {
    var base = '/api/v1';
    var fullUrl = url.startsWith('/api') ? url : base + url;
...
window.loadDoDWidgets = async function loadDoDWidgets() {
  ...
  try {
    ...
    // ATO Countdown
    const expStr = rmf.expiration_date || rmf.expiration || '2026-12-05T00:00:00Z';
    const authStr = rmf.authorization_date || rmf.authorized_at || '2026-06-05T00:00:00Z';
    if (expStr) {
...
window.loadExecutive = async function() {
  ...
  try {
    ...
    const doneRuns = (Array.isArray(runs.runs)?runs.runs:[])
      .filter(r => (r.status==='DONE'||r.status==='PASS'||r.status==='COMPLETED') && r.gate).slice(0,10).reverse();
    const labels = doneRuns.map((r,i) => { const d = r.started_at||r.created_at; return d ? new Date(d).toLocaleDateString('en-GB',{day:'2-digit',month:'short'}) : '#'+(i+1); });
    const scores = doneRuns.map(r => {
      try {
        const s = typeof r.summary==='string'?JSON.parse(r.summary):r.summary;
        if (s && s.SCORE > 0) return s.SCORE;
        const penalty = (s.critical||0)*20 + (s.high||0)*10 + (s.medium||0)*5 + (s.low||0)*1;
        return Math.max(0, Math.min(100, 100 - penalty));
      } catch { return r.gate==='PASS'?100:22; }
    });
    ...
    if (ctx && scores.length) {
      ...
      window._execTrend = new Chart(ctx, {
        ...
        options: { responsive:true, maintainAspectRatio:false, scales:{ x:{ticks:{color:'#64748b',font:{size:9}},grid:{display:false}}, y:{min:0,max:100,ticks:{color:'#64748b',font:{size:9},stepSize:25},grid:{color:'rgba(255,255,255,0.04)'}} }, plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return 'Score: '+c.parsed.y;}}}} }
      });
    }
...
(function startPosturePoller() {
  ...
  async function poll() {
    ...
    try {
      ...
      const latest = _runs.find(x => x.status==='DONE'||x.status==='PASS'||x.status==='COMPLETED');
      ...
      if (elPassRate && _runs.length) {
        const done   = _runs.filter(x=>x.status==='DONE'||x.status==='PASS'||x.status==='COMPLETED');
...
window.vspGetPosture = async function() {
  try {
    ...
    var score = 0; /*SCORE-FIX latest-run*/ try { var _rr = await fetch('/api/v1/vsp/runs?limit=1', {headers:h}).then(function(x){return x.json();}); var _run = _rr && _rr.runs && _rr.runs[0]; var _sm = (_run && (typeof _run.summary==='string'?JSON.parse(_run.summary):_run.summary)) || {}; if (_sm.SCORE != null) score = _sm.SCORE; } catch(_e) { score = 0; }
  ...
  window.showPanel = function(name, btn) {
    ...
    setTimeout(async () => {
      try {
        ...
        if (!sel) return;
        const doneRuns = (runs.runs||[]).filter(r=>r.status==='DONE'||r.status==='PASS'||r.status==='COMPLETED').slice(0,10);
...
(function() {
  function _doLoadRuns() {
    ...
    fetch('/api/v1/vsp/runs/index?limit=200', {
      ...
      var runs = d.runs || [];
      var done = runs.filter(function(r){ return r.status === 'DONE' || r.status === 'PASS' || r.status === 'COMPLETED'; });
...
(function addSBOMDiff(){
  ...
  window._loadSBOMDiffRuns = async function(attempt) {
    ...
    try {
      ...
      var runs = (d.runs||[]).filter(function(r){ return r.status==='DONE'||r.status==='PASS'||r.status==='COMPLETED'; });
...
(function addSLATrendline(){
  ...
  window._renderSLATrend = async function() {
    ...
    try {
      ...
      var runs = (d.runs||[]).filter(function(r){ return r.status==='DONE'||r.status==='PASS'||r.status==='COMPLETED'; }).slice(0,20).reverse();
...
(function(){
  ...
  async function _loadPolicyFull() {
    ...
    try {
      ...
      var [evalD, rulesD] = await Promise.all([
        fetch('/api/v1/policy/status', {method:'GET', headers:h}).then(r=>r.json()).catch(()=>({})),
      ...
      var apiHTML = rules.map(function(r) {
        ...
        return '<div class="rule-card" style="border-left:2px solid var(--cyan)">'
          ...
          +'<div class="rule-name" style="cursor:pointer" onclick="_viewPolicyRule(\''+r.id+'\')" title="Click to view details">'+_vspEsc(r.name)+'</div>'
  ...
  function _vspEsc(s){ return String(s==null?"":s).replace(/[&<>"\x27]/g, function(c){ return {"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","\x27":"&#39;"}[c]; }); }
  window._viewPolicyRule = async function(id) {
    ...
    overlay.innerHTML = '<div class="modal" style="max-width:500px">'
      ...
      +'<div class="modal-title">'+_vspEsc(r.name)+'</div>'
  ...
  window._editPolicyRule = async function(id, btn) {
    ...
    overlay.innerHTML = '<div class="modal" style="max-width:480px"><div class="modal-header">'
      +'<div class="modal-title">Edit rule: '+_vspEsc(rule.name)+'</div>'
      ...
      +'<input id="er-name-gov" class="form-ctrl" value="'+_vspEsc(rule.name)+'"></div>'
      +'<div class="g2 mb8">'
  ...
  window.runEval = async function() {
    ...
    await ensureToken();
    try {
      var d = await fetch('/api/v1/policy/status', {
...
// FIX_SBOM_DIFF_UPGRADE_V1
(function(){
  const MARK = '[FIX-SBOM-DIFF-UPGRADE-V1]';
  if (window.__fix_sbom_diff_upgrade_v1) return;
  window.__fix_sbom_diff_upgrade_v1 = true;

  const origRun = window._runSBOMDiff;
  window._runSBOMDiff = async function() {
    var baseRid = (document.getElementById('sbom-diff-base')||{}).value||'';
    var headRid = (document.getElementById('sbom-diff-head')||{}).value||'';
    if (!baseRid || !headRid) { if(window.showToast) showToast('Chọn 2 runs để compare','error'); return; }
    if (baseRid === headRid) { if(window.showToast) showToast('Chọn 2 runs khác nhau','error'); return; }
    var result = document.getElementById('sbom-diff-result');
    if (result) result.innerHTML = '<div style="color:var(--t3);padding:10px;font-size:11px">Comparing...</div>';
    try {
      await (window.ensureToken ? ensureToken() : Promise.resolve());
      var h = {'Authorization':'Bearer '+(window.TOKEN||localStorage.getItem('vsp_token')||'')};

      // Fetch findings — dùng rid trực tiếp nếu run detail fail
      var safeJson = function(p){ return p.then(function(r){ return r.ok ? r.json() : {}; }).catch(function(){ return {}; }); };

      var baseRun = await safeJson(fetch('/api/v1/vsp/run/'+baseRid, {headers:h}));
      var headRun = await safeJson(fetch('/api/v1/vsp/run/'+headRid, {headers:h}));

      // Fallback: dùng rid nếu id không có
      var baseId = baseRun.id || baseRun.run_id || baseRid;
      var headId = headRun.id || headRun.run_id || headRid;

      var baseFd = await safeJson(fetch('/api/v1/vsp/findings?run_id='+encodeURIComponent(baseId)+'&limit=2000', {headers:h}));
      var headFd = await safeJson(fetch('/api/v1/vsp/findings?run_id='+encodeURIComponent(headId)+'&limit=2000', {headers:h}));

      var baseF = baseFd.findings || baseFd.items || [];
      var headF = headFd.findings || headFd.items || [];
      ...
      var sevClass = {CRITICAL:'sev-crit',HIGH:'sev-high',MEDIUM:'sev-med',LOW:'sev-low'};
      var mkRows = function(arr, color, label) {
        if (!arr.length) return '<div style="font-size:11px;color:var(--t3);padding:6px 0">No ' + label.toLowerCase() + '</div>';
        return '<div style="margin-bottom:10px">'
          +'<div style="font-size:10px;font-weight:700;color:'+color+';letter-spacing:.08em;margin-bottom:6px">'+label+' ('+arr.length+')</div>'
          +'<div style="display:grid;gap:3px">'
          +arr.slice(0,10).map(function(f){
            return '<div style="display:flex;gap:8px;align-items:center;padding:4px 8px;background:'+color+'11;border-left:2px solid '+color+';border-radius:0 4px 4px 0">'
              +'<span class="sev '+(sevClass[f.severity]||'')+'">'+((f.severity||'?').slice(0,4))+'</span>'
              +'<span style="font-size:10px;color:var(--t3);flex-shrink:0">'+(f.tool||'')+'</span>'
              +'<span style="font-size:11px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(f.message||f.rule_id||'')+'</span>'
              +'<span style="font-size:10px;color:var(--t3)">'+((f.path||'').split('/').pop())+'</span>'
              +'</div>';
          }).join('')
          +(arr.length>10?'<div style="font-size:10px;color:var(--t3);padding:4px 8px">…and '+(arr.length-10)+' more</div>':'')
          +'</div></div>';
      ...
      if (result) result.innerHTML =
        '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px">'
        +'<div style="padding:10px;text-align:center;background:rgba(239,68,68,.08);border-radius:6px"><div style="font-size:10px;color:var(--t3)">New findings</div><div style="font-size:24px;font-weight:700;color:#ef4444">+'+added.length+'</div></div>'
        +'<div style="padding:10px;text-align:center;background:rgba(34,197,94,.08);border-radius:6px"><div style="font-size:10px;color:var(--t3)">Fixed</div><div style="font-size:24px;font-weight:700;color:#22c55e">-'+removed.length+'</div></div>'
        +'<div style="padding:10px;text-align:center;background:var(--bg2);border-radius:6px"><div style="font-size:10px;color:var(--t3)">Unchanged</div><div style="font-size:24px;font-weight:700;color:var(--t2)">'+unchanged.length+'</div></div>'
        +'</div>'
        +mkRows(added, '#ef4444', '▲ NEW FINDINGS')
        +mkRows(removed, '#22c55e', '▼ FIXED')
        +(unchanged.length?'<div style="font-size:10px;color:var(--t3);padding:6px 0">'+unchanged.length+' findings unchanged</div>':'');

      if(window.showToast) showToast('Diff: +'+added.length+' new, -'+removed.length+' fixed','success');
      console.log(MARK, 'diff OK — added:', added.length, 'removed:', removed.length);
    } catch(e) {
      console.error(MARK, 'diff error:', e);
      if (result) result.innerHTML = '<div style="color:#ef4444;padding:10px;font-size:11px">✗ Error: '+e.message+'</div>';
    }
  };

  console.log(MARK, 'installed — _runSBOMDiff patched with null-safe checks');
})();
// /FIX_SBOM_DIFF_UPGRADE_V1
