...
(function () {
  ...
  async function api(path, opts) {
    // __VSP_SCHED_WRITE_XHR_V1__  (writes bypass fetch-wrappers via XHR -> gateway)
    var __vm = (opts && opts.method ? String(opts.method) : 'GET').toUpperCase();
    if (__vm !== 'GET' && __vm !== 'HEAD') {
      if (!window.__vsp_sched_xhr_logged){window.__vsp_sched_xhr_logged=1;console.log('[__VSP_SCHED_WRITE_XHR_V1__] active');}
      var __tok = (window.TOKEN || localStorage.getItem('vsp_token') || '');
      return new Promise(function (resolve, reject) {
        var xhr = new XMLHttpRequest();
        xhr.open(__vm, '/api/sched' + path, true);
        xhr.setRequestHeader('Content-Type', 'application/json');
        if (__tok) xhr.setRequestHeader('Authorization', 'Bearer ' + __tok);
        xhr.onload = function () {
          var ct = (xhr.getResponseHeader('content-type') || '');
          var body = xhr.responseText;
          if (ct.indexOf('json') >= 0) { try { body = JSON.parse(xhr.responseText); } catch (e) {} }
          if (xhr.status >= 200 && xhr.status < 300) { resolve(body); }
          else { reject(new Error((body && body.error) ? body.error : ('HTTP ' + xhr.status))); }
        };
    ...
    try {
      const r = await fetch(API_BASE + path, Object.assign({
        headers: { 'Content-Type': 'application/json', ...(( window.TOKEN || localStorage.getItem('vsp_token') || '') ? { 'Authorization': 'Bearer ' + (window.TOKEN || localStorage.getItem('vsp_token') || '') } : {}) },
        signal:  ctrl.signal,
      }, opts || {}));
      const ct = r.headers.get('content-type') || '';
      const body = ct.includes('json') ? await r.json() : await r.text();
      if (r.status === 401 || r.status === 403) {
        // Token expired — refresh from window.TOKEN
        const freshTok = window.TOKEN || localStorage.getItem('vsp_token') || '';
        if (freshTok) {
          const r2 = await nativeFetch(API_BASE + path, Object.assign({
          ...
          if (!r2.ok && typeof body2 === 'object') throw new Error(body2.error || ('HTTP ' + r2.status));
                  ...
                  <span id="sch-m-weekly-d" style="display:inline-flex;gap:3px;margin:0 4px">
                    ${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((d,i)=>`
                      <label style="font-size:10px;cursor:pointer;padding:2px 6px;border:1px solid var(--border);border-radius:3px;background:var(--bg2)">
                        <input name="dow" aria-label="Dow" type="checkbox" data-dow="${i}" ${i===1?'checked':''} style="display:none">${d}
                      </label>
                    `).join('')}
                  </span>
