...
(function () {
  ...
  async function api(path, opts) {
    ...
    try {
      /* SWINV-AUTH-FIX: dinh Bearer tu vsp_token/window.TOKEN (iframe same-origin) */
      var __tok = window.TOKEN || (function(){try{return localStorage.getItem('vsp_token')||''}catch(e){return ''}})();
      var __hdr = Object.assign({ 'Content-Type': 'application/json' }, (opts && opts.headers) || {});
      if (__tok && !__hdr['Authorization'] && !__hdr['authorization']) __hdr['Authorization'] = 'Bearer ' + __tok;
      const r = await fetch(API_BASE + path, Object.assign({}, opts || {}, { headers: __hdr, signal: ctrl.signal }));
